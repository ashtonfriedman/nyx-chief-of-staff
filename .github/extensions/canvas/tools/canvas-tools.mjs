...
function openBrowser(url) {
  if (process.platform === "win32") {
    spawn("cmd", ["/c", "start", "msedge", url], { stdio: "ignore", windowsHide: true });
  } else if (process.platform === "darwin") {
    spawn("open", ["-a", "Microsoft Edge", url], { stdio: "ignore" });
  } else {
  ...
  return [
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          port: {
            type: "number",
            description: "Optional fixed port (e.g. 9999). If omitted, a random available port is used. Fixed ports produce stable, bookmarkable URLs.",
          },
        },
        required: ["name"],
      },
      handler: async (args) => {
        ...
        if (!/^[a-z0-9-]+$/.test(args.name)) {
          return "Error: canvas name must be kebab-case (lowercase letters, numbers, hyphens only).";
        }
...
${html}
        ...
        // Start server if not running (use fixed port if specified)
