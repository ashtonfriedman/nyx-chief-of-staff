...
// Permissions are scoped — only approved tools are auto-accepted.
...
const PERMITTED_TOOLS = new Set([
  // Code-exec tools (read MCP data sources)
  "discover_mcp_tools", "call_mcp_tool", "execute_script",
  // Read-only exploration
  "view", "grep", "glob", "sql", "web_search", "web_fetch",
  // Calendar/mail read (no send/create)
  "calendar-ListCalendarView", "calendar-ListEvents",
  "calendar-GetUserDateAndTimeZoneSettings",
  "mail-SearchMessages", "mail-GetMessage", "mail-GetAttachments",
  // Teams read
  "teams-ListChats", "teams-ListChatMessages", "teams-ListChannelMessages",
...
/**
 * Scoped permission handler — only approves tools in the permitted set.
 * Denies everything else and logs the denial.
 */
function scopedPermissions(request) {
  const toolName = request?.toolName || request?.name || request?.tool?.name || "";
  if (PERMITTED_TOOLS.has(toolName)) {
    return true;
  }
  process.stderr.write(
    `[prompt-executor] DENIED tool request: ${toolName}\n`
  );
  return false;
}
...
export async function executePrompt(extDir, payload) {
  ...
  try {
    ...
    const sessionOpts = {
      onPermissionRequest: scopedPermissions,
    };
    if (payload.model) {
