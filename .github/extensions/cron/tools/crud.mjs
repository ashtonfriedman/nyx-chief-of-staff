...
/** Append an immutable audit entry for cron CRUD operations. */
function appendAuditLog(extDir, entry) {
  const dataDir = getDataDir(extDir);
  mkdirSync(dataDir, { recursive: true });
  const line = JSON.stringify({
    timestamp: new Date().toISOString(),
    ...entry,
  });
  appendFileSync(join(dataDir, "audit.jsonl"), line + "\n", "utf-8");
}
  ...
  return [
    {
      ...
      handler: async (args) => {
        ...
        writeJob(extDir, job);
        appendAuditLog(extDir, { operation: "create", jobId: id, jobName: args.name });
...

    {
      ...
      handler: async (args) => {
        ...
        writeJob(extDir, job);
        appendAuditLog(extDir, { operation: "update", jobId: args.jobId, jobName: job.name, fields: changes });
...

    {
      ...
      handler: async (args) => {
        ...
        const name = job.name;
        appendAuditLog(extDir, { operation: "delete", jobId: args.jobId, jobName: name });
