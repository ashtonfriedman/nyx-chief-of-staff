...
import { listJobs, readJob, writeJob, verifyJob } from "../lib/store.mjs";
...
async function dispatch(job) {
  ...
  // Verify job signature before execution
  const verification = verifyJob(extDir, job);
  if (!verification.valid) {
    if (verification.reason === "unsigned") {
      log(`WARNING: Job ${job.id} is unsigned (legacy). Re-sign by updating it via cron_update.`);
    } else {
      log(`REJECTED: Job ${job.id} failed signature verification: ${verification.reason}`);
      concurrentCount--;
